...
var randomWord = require('./puzzle-word');
...
function getPuzzle(puzzleId, callback) {
	db.collection('puzzles').findOne({ _id : ObjectID.createFromHexString(puzzleId) }, function(err, result) {
        if (err) throw err;
        callback(result);
    });
};
